...
// Follows FIPS 197: byte 0 is state[127:120], byte index is 4*column + row
...
function automatic logic [127:0] aes_sub_bytes(input logic [127:0] value);
 logic [127:0] result;
 for (int i = 0; i < 16; i++;) begin
    result[127 - 8*i -: 8] = aes_sbox(state[127-8*i -: 8]);
 end
 return result;
endfunction

function automatic logic [127:0] aes_inv_sub_bytes(input logic [127:0] value);
 logic [127:0] result;
 for (int i = 0; i < 16; i++;) begin
    result[127 - 8*i -: 8] = aes_inv_sbox(state[127-8*i -: 8]);
 end
 return result;
endfunction

function automatic logic [127:0] aes_shift_rows(input logic [127:0] state);
   // output byte indices: 0,5,10,15, 4,9,14,3, 8,13,2,7, 12,1,6,11
 return {
...
function automatic logic [127:0] aes_inv_shift_rows(input logic [127:0] state);
    // output byte indices: 0,13,10,7, 4,1,14,11, 8,5,2,15, 12,9,6,3
 return {
